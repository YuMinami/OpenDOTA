...
public class ChannelLifecycleController {
    ...
    @PostMapping("/open")
    public ApiResponse<Map<String, String>> open(@RequestBody ChannelOpenRequest req, HttpServletRequest http) {
        validate(req);
    ...
    public ApiResponse<Map<String, String>> close(@PathVariable String channelId, HttpServletRequest http) {
    ...
    private static void validate(ChannelOpenRequest req) {
        if (req == null) throw new DispatchException(ErrorCodes.BAD_REQUEST, "请求 body 必填");
        if (req.vin() == null || req.vin().length() != 17) {
            throw new DispatchException(ErrorCodes.BAD_REQUEST, "vin 必须为 17 位");
        }
        if (req.ecuName() == null || req.ecuName().isBlank()) {
            throw new DispatchException(ErrorCodes.BAD_REQUEST, "ecuName 必填");
        }
        if (req.ecuScope() == null || req.ecuScope().isEmpty()) {
            throw new DispatchException(ErrorCodes.BAD_REQUEST, "ecuScope 必填(R2 约束,单 ECU 场景填 [ecuName])");
        }
        if (req.txId() == null || req.rxId() == null) {
            throw new DispatchException(ErrorCodes.BAD_REQUEST, "txId / rxId 必填");
        }
    }
